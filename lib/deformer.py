...
class SMPLDeformer():
    def __init__(self, max_dist=0.1, K=1, gender='female', betas=None):
        ...
        smpl_params_canoical[:, -10:] = betas.clone().detach().float().to(self.smpl.param_canonical.device)
        #cano_scale, cano_transl, cano_thetas, cano_betas = torch.split(smpl_params_canoical, [1, 3, 72, 10], dim=1)
        smplx=False
        if smplx:
            cano_scale, cano_transl, cano_orient, cano_thetas, cano_betas = torch.split(smpl_params_canoical, [1, 3, 3, 63, 10], dim=1)
        else:
            cano_scale, cano_transl, cano_orient, cano_thetas, cano_betas = torch.split(smpl_params_canoical, [1, 3, 3, 69, 10], dim=1)
        #cano_scale, cano_transl, cano_thetas, cano_betas, expression = torch.split(smpl_params_canoical, [1, 3, 165, 10, 10], dim=1)
        smpl_output = self.smpl(cano_scale, cano_transl, cano_orient, cano_thetas, cano_betas)
