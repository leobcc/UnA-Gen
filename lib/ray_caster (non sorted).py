...
class RayCaster:
    ...
    def render_mask_pruning(self, dynamical_voxels_world_coo, voxels_ov, voxels_uv, image, depth_image):
        ...
        occupied_pixels_mask = voxels_ov.detach() * occupied_pixels_mask
    ...
    def render_rays(self, dynamical_voxels_world_coo, voxels_ov, voxels_rgb, voxels_uv, image, depth_image, rays_uv=None):
        ...
        if self.opt['ray_cast_rgb']:
            ...
            assert self.opt['closeness_threshold'] > 0, "Closeness threshold must be greater than 0"
            ...
            distances_selected = distances_selected / threshold
            distances_selected = (~mask) + distances_selected   # far voxels have distance 1

            norm_dis = depth * distances_selected
        ...
        if self.opt['ray_cast_depth'] and self.mode == 'training':
            ...
            image_mask = (image.sum(dim=1) > 0).float()
            depth_image = depth_image * image_mask
