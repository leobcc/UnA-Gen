...
def get_center_point(num_cams, cameras):
    ...
    for i in range(num_cams):
        # Get the intrinsic matrix K and the extrinsic matrix RT
        K = cameras['K_%d' % i]
        RT = cameras['RT_%d' % i]

        # Use only the first 3 rows of RT (to get the 3x4 part)
        P0 = K @ RT[:3, :]  # Compute the projection matrix P = K * (RT[:3, :])

        _, R, c = cv2.decomposeProjectionMatrix(P0)[:3]
        c = c / c[3]  # Normalize the homogeneous coordinates
...
def normalize_cameras(original_cameras_filename, output_cameras_filename, num_of_cameras, scene_bounding_sphere=3.0):
    ...
    # Get the center points of the cameras
    print("number of cameras: ", num_of_cameras)
    ...
    for i in range(num_of_cameras):
        ...
        cameras_new['world_mat_%d' % i] = np.hstack((cameras['K_%d' % i], cameras['RT_%d' % i][:3, -1:]))  # Combine K and T (from RT)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_cameras_file', type=str, default="cameras.npz",
                        help='the input cameras file (should contain K and RT matrices)')
